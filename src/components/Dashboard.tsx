...
export default function Dashboard() {
    ...
    return (
        <div className="space-y-6">
            {/* Month Selector */}
            <div className="flex items-center justify-center gap-4 bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <button
                ...
                <div className="text-center min-w-[200px]">
                    ...
                    <p className="text-xs text-gray-500">Período visualizado</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {cards.map((card, i) => (
                    <div key={i} className={`bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow`}>
                        ...
                        <p className={`text-2xl font-bold mt-1 ${card.color === 'emerald' ? 'text-emerald-600' :
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-emerald-500">
                    <p className="text-gray-500 text-sm font-medium">Gastos Ejecutados / Pagados</p>
                    <p className="text-2xl font-bold text-emerald-600 mt-1">{formatMoney(data.currentMonth.executed)}</p>
                    <p className="text-xs text-gray-400 mt-1">Reales + TC Pagadas</p>
                </div>
                <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-amber-500">
                    <p className="text-gray-500 text-sm font-medium">Gastos Pendientes (Proyectados)</p>
                    <p className="text-2xl font-bold text-amber-600 mt-1">{formatMoney(data.currentMonth.pending)}</p>
                    <p className="text-xs text-gray-400 mt-1">TC a vencer</p>
                </div>
                <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-blue-500">
                    <p className="text-gray-500 text-sm font-medium">Saldo Remanente Real</p>
                    <p className={`text-2xl font-bold mt-1 ${data.currentMonth.remaining >= 0 ? 'text-blue-600' : 'text-rose-600'}`}>
                        {formatMoney(data.currentMonth.remaining)}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">Ingresos - Total (Ejec + Pend)</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-gray-600 text-sm">{data.currentMonth.label}</span>
                            <span className={`font-semibold ${data.currentMonth.balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {formatMoney(data.currentMonth.balance)}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-gray-600 text-sm">{data.previousMonth.label}</span>
                            <span className={`font-semibold ${data.previousMonth.balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {formatMoney(data.previousMonth.balance)}
                            </span>
                        </div>
                        <div className="pt-2 border-t">
                            <div className="flex justify-between items-center">
                                <span className="text-gray-500 text-sm">Variación</span>
                                <span className={`font-bold ${data.monthlyVariation.balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-gray-500 text-xs uppercase">Esta Semana</p>
                            <p className="text-emerald-600 font-semibold">{formatMoney(data.currentWeek.income)}</p>
                            <p className="text-rose-600 font-semibold">{formatMoney(data.currentWeek.expense)}</p>
                        </div>
                        <div>
                            <p className="text-gray-500 text-xs uppercase">Semana Anterior</p>
                            <p className="text-emerald-600 font-semibold">{formatMoney(data.previousWeek.income)}</p>
                            <p className="text-rose-600 font-semibold">{formatMoney(data.previousWeek.expense)}</p>
                        </div>
                    </div>
                    <div className="mt-3 pt-3 border-t flex justify-between">
                        <span className="text-gray-500 text-sm">Var. Gastos</span>
                        <span className={`font-bold ${data.weeklyVariation.expense <= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        ...
                        {data.topExpenses.length === 0 ? (
                            ...
                            data.topExpenses.map((item: any, i: number) => (
                                <div key={i} className="flex justify-between items-center">
                                    <span className="text-gray-600 text-sm truncate flex-1">{item.category}</span>
                                    <span className="text-rose-600 font-medium ml-2">{formatMoney(item.amount)}</span>
                                </div>
                            ))
                        )}
                    </div>
                        ...
                        <tbody>
                            {data.monthlyHistory.map((month: any, i: number) => (
                                <tr key={i} className="border-t border-gray-100">
                                    <td className="py-2 font-medium text-gray-700 capitalize">{month.label}</td>
                                    <td className="py-2 text-right text-emerald-600">{formatMoney(month.income)}</td>
                                    <td className="py-2 text-right text-rose-600">{formatMoney(month.expense)}</td>
                                    <td className={`py-2 text-right font-semibold ${month.balance >= 0 ? 'text-blue-600' : 'text-rose-600'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {data.accounts.map((account: any, i: number) => (
                            <div key={i} className="bg-gray-50 rounded-lg p-4">
                                ...
                                <p className={`text-xl font-bold ${account.balance >= 0 ? 'text-gray-800' : 'text-rose-600'}`}>
            ...
            {data.categoryBreakdown && data.categoryBreakdown.length > 0 && (
                <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                                ...
                                {data.categoryBreakdown.filter((c: any) => !c.isTC).map((cat: any, i: number) => {
                                    ...
                                    return (
                                        <div key={i} className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm font-medium text-gray-700 w-24 text-right">{formatMoney(cat.amount)}</span>
                                        </div>
                                    );
                                })}
                                <div className="pt-2 border-t mt-2 flex justify-between">
                                    <span className="font-medium text-gray-700">Total sin TC</span>
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                ...
                                {data.categoryBreakdown.filter((c: any) => c.isTC).length === 0 && (
                                    <p className="text-gray-400 text-sm">Sin gastos de TC este mes</p>
                                )}
                                <div className="pt-2 border-t mt-2 flex justify-between">
                                    <span className="font-medium text-gray-700">Total TC</span>
                    ...
                    <div className="mt-4 pt-4 border-t-2 border-gray-200 flex justify-between items-center">
                        <span className="font-semibold text-gray-800">Total General del Mes</span>
